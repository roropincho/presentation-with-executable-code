...
.md.html: webapp-tutorial.js webapp-tutorial.css
	...
	pandoc +RTS -M512M -K128M -RTS -H webapp-tutorial.temp -f markdown -t revealjs -V progress:true -V slideNumber:true -s -o $*.html $*.md --slide-level=1 --template=custom-template-reveal.html
...
.scm.js:
	@if ../gambit/gsc/gsc -target js -o $*.js -exe $*.scm; then \
